...
snake_algo = importlib.import_module('snake-algo-code')
...
CELL_SIZE = 30
# Frame rate cap (images par seconde) — limite le rendu
GAME_SPEED = 100
# Snake movement speed (mouvements par seconde). Augmentez pour accélérer le serpent.
SNAKE_SPEED = 300
...
# If True the game will use the algorithm for automatic moves (not ML/IA, just path algo)
AUTO_MODE = True
...
def main():
    ...
    # Accumulateur temporel pour la gestion du mouvement du serpent (secondes)
    move_accumulator = 0.0

    # --- Boucle de jeu ---
    while running:
        # Delta time et limitation du rendu
        ...
        if not game_over and not victory:
            # intervalle entre deux mouvements du serpent
            step_interval = 1.0 / max(1e-6, SNAKE_SPEED)
            # effectuer autant de pas que nécessaire selon l'accumulateur
            while move_accumulator >= step_interval:
                move_accumulator -= step_interval

                # Si AUTO_MODE activé, demander la direction à l'algorithme
                if AUTO_MODE:
                    try:
                        next_dir = snake_algo.get_next_direction(snake.body, apple.position, GRID_SIZE)
                        if next_dir:
                            snake.set_direction((int(next_dir[0]), int(next_dir[1])))
                    except Exception:
                        pass
                ...
                if snake.is_game_over():
                    ...
                    if victory:
                        print(f"VICTOIRE en {elapsed:.2f}s - Score: {snake.score}")
                    else:
                        print(f"GAME OVER en {elapsed:.2f}s - Score: {snake.score}")
                    break

...
# --- FONCTION POUR DEFINIR LA LOGIQUE DU SERPENT ---

def compute_next_direction(snake: Snake, apple: Apple):
    # Helpers
    def to_tuple(p): return (p[0], p[1])
    start = to_tuple(snake.head_pos)
    if not apple.position:
        return snake.direction
    target = to_tuple(apple.position)

    # Build obstacles set (use tuples). allow stepping into current tail if it will vacate.
    obstacles = set(to_tuple(p) for p in snake.body)
    tail = to_tuple(snake.body[-1]) if snake.body else None
    if not snake.grow_pending and tail:
        obstacles.discard(tail)

    if start == target:
        return snake.direction

    # Toroidal neighbors & distance
    def wrap(p): return (p[0] % GRID_SIZE, p[1] % GRID_SIZE)
    def neighbors(node):
        for d in (UP, DOWN, LEFT, RIGHT):
            yield wrap((node[0] + d[0], node[1] + d[1]))
    def toroidal_dist(a, b):
        dx = abs(a[0] - b[0]); dx = min(dx, GRID_SIZE - dx)
        dy = abs(a[1] - b[1]); dy = min(dy, GRID_SIZE - dy)
        return dx + dy

    # A* search
    open_heap = [(toroidal_dist(start, target), 0, start)]
    gscore = {start: 0}
    came_from = {}
    closed = set()
    while open_heap:
        ...
        for n in neighbors(current):
            if n in closed or n in obstacles:
                continue
            ng = g + 1
            if ng < gscore.get(n, 1_000_000):
                gscore[n] = ng
                priority = ng + toroidal_dist(n, target)
                came_from[n] = current
                heapq.heappush(open_heap, (priority, ng, n))

    # no path found -> safe fallback (prefer current direction)
    def safe_fallback():
        for d in (snake.direction, UP, RIGHT, DOWN, LEFT):
            if (d[0]*-1, d[1]*-1) == snake.direction:
                continue
            nxt = wrap((start[0] + d[0], start[1] + d[1]))
            if nxt not in obstacles:
                return d
        # last resort: keep direction
        return snake.direction

    if target not in came_from and target != start:
        return safe_fallback()

    # reconstruct path (list of nodes from start->target)
    path = []
    cur = target
    while cur != start:
        path.append(cur)
        ...
        if cur is None:
            return safe_fallback()
    path.reverse()  # first step is path[0]

    first_step = path[0]

    # Safety check: simulate movement along the path and ensure after eating apple
    # there's a path from the apple to the tail (prevents getting trapped).
    sim_body = [to_tuple(p) for p in snake.body]
    ate = False
    for step in path:
        sim_body.insert(0, step)
        if step == target:
            ate = True
            # when apple eaten, snake grows (don't pop tail this turn)
        else:
            # normal move: tail vacates
            sim_body.pop()
    # After simulation, compute tail position and obstacles for reachability check
    sim_tail = sim_body[-1]
    sim_obstacles = set(sim_body)
    # If tail will move after some steps, allow stepping onto it:
    if sim_tail in sim_obstacles:
        sim_obstacles.discard(sim_tail)

    # BFS from apple (target) to simulated tail
    def reachable(a, b, block):
        q = deque([a]); seen = {a}
        while q:
            u = q.popleft()
            if u == b:
            ...
            for n in neighbors(u):
                if n in seen or n in block:
                    continue
                seen.add(n); q.append(n)
        return False

    if reachable(target, sim_tail, sim_obstacles):
        # Convert first_step to direction and prevent immediate reverse
        for d in (UP, DOWN, LEFT, RIGHT):
            if wrap((start[0] + d[0], start[1] + d[1])) == first_step:
                if (d[0] * -1, d[1] * -1) == snake.direction:
                    return snake.direction
                return d
        return snake.direction

    # If safety check failed, fallback to a safe neighbor maximizing open space
    def reachable_count_from(cell, block, cap=GRID_SIZE*GRID_SIZE):
        q = deque([cell]); seen = {cell}; cnt = 0
        while q and cnt < cap:
            u = q.popleft(); cnt += 1
            for n in neighbors(u):
                if n in seen or n in block:
    ...
    for d in (UP, RIGHT, DOWN, LEFT):
        ...
        if nxt in obstacles:
            continue
        score = reachable_count_from(nxt, obstacles)
        if score > best_score:
...
def main():
    ...
    # --- Boucle de jeu ---
    while running:
        # 0. Algorithme pour determiner la direction du serpet
        snake.set_direction(compute_next_direction(snake, apple))

...
class Snake:
    ...
    def move(self):
        """Déplace le serpent d'une case, avec ou sans wrap selon WRAP."""
        if WRAP:
...
# === Pathfinding solver (A*) - Version simple et déterministe ==================
...
def astar(start, goal, blocked):
    """A* basique sur une grille toroïdale."""
...
def get_algo_direction(snake, apple):
    ...
    # 1️⃣ Essaye A* vers la pomme
    path = astar(head, apple_pos, blocked)
    if path and len(path) > 1:
        # Calcule le premier mouvement à faire
    ...
    # 2️⃣ Pas de chemin : avance sans mourir (ordre fixe donc déterministe)
    for d in DIRS:
        nx, ny = torus_add(head[0], head[1], d)
        if [nx, ny] not in snake.body:
            return d

    # 3️⃣ Aucune issue → crash inévitable
...
def main():
    ...
    print(f"  Durée de la partie : {int(time.time() - start_time)} secondes")
    if victory:
